	...
	return defineComponent({
		...
		methods: {
			...
			async getBussinessComponent() {
				this.BussinessComponentSourceCode = await _.$SourceCode_SFC(
					_.$resolvePath(this.path)
				);
				this.rerun();
			},
			async showSourceCodeDialog() {
				const WindowImageModify = await _.$importVue(
					"@/components/WindowSourceCode.vue",
					{
						$parent: this,
		...
		render() {
			...
			return h(
				"div",
				{ staticClass: "padding10", style: this.styleContainer },
				[
					h(
						"div",
						{
