...
exports.copyNewProject = async function (name) {
	...
	await asyncCpDir(
		path.resolve(__dirname, "../..", `static_vue2/business_a_init_element-ui`),
		projectRootDir
	);
	let content = await fs.promises.readFile(
		path.resolve(projectRootDir, "./utils/api.vue"),
		"utf-8"
	);
	await fs.promises.writeFile(
		path.resolve(projectRootDir, "./utils/api.vue"),
		content.replaceAll("_api.xxxxxxx", `_api.${name}`),
		"utf-8"
	);

	content = await fs.promises.readFile(path.resolve(projectRootDir, "./utils/opts.vue"), "utf-8");
	await fs.promises.writeFile(
		path.resolve(projectRootDir, "./utils/opts.vue"),
		content.replaceAll("_opts.xxxxxxx", `_opts.${name}`),
		"utf-8"
	);
	...
	await fs.promises.writeFile(
		path.resolve(projectRootDir, `./${name}.html`),
		content.replaceAll("xxxxxxx", name),
		"utf-8"
	);
